...
export class SnomedService {
  ...
  readonly baseUrl: string = "http://localhost:4200/snowstorm/fhir/ValueSet/$expand?url=http://snomed.info/sct/45991000052106?";
  ...
  search(term: string, ecl: string): any {
    ...
    if (term && term.length > 0) {
      return this.http.get('http://localhost:4200/snowstorm/fhir/ValueSet/$expand?url=http://snomed.info/sct/45991000052106?fhir_vs=ecl/' +
  ...
  getMap(conceptId: string, mapId: string) {
    ...
    if (conceptId && conceptId.length > 0) {
      return this.http.get('http://localhost:4200/snowstorm/MAIN/SNOMEDCT-SE/MAPTEST1/members?referenceSet=' +
